...
describe('Selector indicator (chevron) icon theme target', () => {
  ...
  it('reflects the disabled state on the selector target', () => {
    const {container} = render(
      <Selector
        label="Fruit"
        options={OPTIONS}
  ...
  it('does not reflect disabled when enabled', () => {
    const {container} = render(
      <Selector label="Fruit" options={OPTIONS} onChange={() => {}} />,
    );
    expect(container.querySelector('.astryx-selector')).not.toHaveAttribute(
      'data-disabled',
    );
  });
});
